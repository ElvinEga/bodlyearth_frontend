...
export default function EngagementTable({ scoreList }: Props) {
  const [riskData, setRiskData] = useState<RiskDataById>();
  ...
  const onPreviewScore = async (scoreId: string) => {
    Swal.showLoading();
    const RISK_URL = `/risk/v1/location_scores/${scoreId}/score`;
    // setIsLoading(true);
    await axiosPrivate<RiskDataById>({
      method: "GET",
      url: RISK_URL,
    })
      .then((data) => {
                    ...
                    <tr>
                      ...
                      <td className="h-px w-px whitespace-nowrap">
                        <div className="px-6 py-1.5">
                          <button
                            className="ml-3"
                            onClick={() => onPreviewScore(data.score_id)}
                          >
                ...
                <div className="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all lg:max-w-4xl lg:w-full m-3 lg:mx-auto">
                  <div className="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div className="p-4 overflow-y-auto">
                      <PdfPreviewComponent myRiskdata={riskData} />
                    </div>
                  </div>
                </div>
              </div>
